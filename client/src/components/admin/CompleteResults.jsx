...
const CompleteResults = ({ score, activeQuiz, studentName, onRetake }) => {
  ...
  // Badge images
  const BADGES = {
    GOLD: process.env.PUBLIC_URL + '/gold-f.jpg',
    SILVER: process.env.PUBLIC_URL + '/siver-f.jpg',
    BRONZE: process.env.PUBLIC_URL + '/bronze-f.jpg'
  };
  ...
  useEffect(() => {
    console.log('🖼️ Badge paths being used:', BADGES);
    console.log('📁 Public URL:', process.env.PUBLIC_URL);
  }, []);


  // Function to save badge to localStorage
  // const saveBadgeToStorage = (badgeType, quizName, score, date) => {
  //   try {
  //     const badgeData = {
  //       type: badgeType,
  //       quizName: quizName,
  ...
  //     // Get existing badges or initialize empty array
  //     const existingBadges = JSON.parse(localStorage.getItem('studentBadges') || '[]');

  //     // Check if badge already exists for this quiz
  //     const existingBadgeIndex = existingBadges.findIndex(badge => 
  //       badge.quizName === quizName && badge.type === badgeType
  //     );

  //     if (existingBadgeIndex === -1) {
  //       // Add new badge
  //       existingBadges.push(badgeData);
  //       localStorage.setItem('studentBadges', JSON.stringify(existingBadges));
  //       console.log('🏅 Badge saved to localStorage:', badgeType);
  //     } else {
  //       // Update existing badge
  //       existingBadges[existingBadgeIndex] = badgeData;
  //       localStorage.setItem('studentBadges', JSON.stringify(existingBadges));
  //       console.log('🏅 Badge updated in localStorage:', badgeType);
  //     }

  //     return true;
  //   } catch (error) {
  //     console.error('❌ Error saving badge to localStorage:', error);
  //     return false;
  //   }
  // };
  ...
  const getBadgeForRank = (rank) => {
    switch (rank) {
      case 1:
        return { type: 'GOLD', image: BADGES.GOLD, title: 'Gold Badge' };
      case 2:
        return { type: 'SILVER', image: BADGES.SILVER, title: 'Silver Badge' };
      case 3:
        return { type: 'BRONZE', image: BADGES.BRONZE, title: 'Bronze Badge' };
      default:
        return null;
    }
  };
  ...
  useEffect(() => {
    const loadCompleteResults = async () => {
      ...
      try {
        ...
        // Determine and save badge for current user
        const userBadgeInfo = getBadgeForRank(calculatedRank);
        if (userBadgeInfo) {
          setUserBadge(userBadgeInfo);
          console.log(`🎉 ${studentName} earned a ${userBadgeInfo.type} badge!`);
        }
        console.log(`👤 User ${studentName} rank: ${calculatedRank} of ${sortedResults.length}`);
  ...
  return (
    <div className="complete-results">
      <div className="results-container">
        ...
        {/* Badge Congratulations Card */}
        {userBadge && (
          <div className="badge-congratulations">
            <div className="badge-header">
              <h2>🎉 Congratulations! 🎉</h2>
              <p>You earned a {userBadge.title}!</p>
            </div>
            <div className="badge-display">
              ...
              {/* <div className="badge-info">
                <h3>{userBadge.title}</h3>
                <p>Rank #{userRank} in {activeQuiz?.name}</p>
                <p>Score: {score}/{totalQuestions} ({percentage}%)</p>
                <small>Badge saved to your collection!</small>
              </div> */}
            </div>
          </div>
        )}
        ...
        {/* Personal Score Card */}
        <div className={`personal-score-card ${userBadge ? 'with-badge' : ''}`}>
          <h3>Your Performance</h3>
          <div className="score-display">
            <div className="score-circle">
              <span className="score">{score}</span>
        ...
        <div className="ranking-section">
          ...
          <div className="rankings-list">
            {allResults.map((result, index) => {
              ...
              const resultTotalQuestions = result.totalQuestions || totalQuestions;
              const resultPercentage = Math.round((result.score / resultTotalQuestions) * 100);
              const resultBadge = getBadgeForRank(index + 1);
                ...
                >
                  ...
                  {resultBadge && (
                    <div className="badge-indicator">
                      <img
                        src={resultBadge.image}
                        alt={resultBadge.title}
      ...
      <style jsx>{`
        ...
        .personal-score-card.with-badge {
          background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
        }
        ...
        /* Add your existing responsive styles below */
      `}</style>
    </div>
  );
};
